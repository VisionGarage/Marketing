#!/usr/bin/env python3
"""Core FSC generation logic (RSA e=3 suffix-forgery over a weak MD5 verifier)."""
...
def load_template(data: bytes | None) -> bytearray:
    payload = BUILTIN_TEMPLATE if data is None else data
    expected = BODY_LEN + SIGNATURE_LEN
    if len(payload) < expected:
        raise ValueError(
            f"template is {len(payload)} bytes; need at least {expected} bytes"
        )
    return bytearray(payload[:BODY_LEN] + payload[-SIGNATURE_LEN:])
...
def build_fsc(
    ...
    if len(vin) != VIN_LEN:
        raise ValueError("VIN must be exactly 7 ASCII characters")
...
def validate_vin(vin7: str) -> bytes:
    try:
        vin = vin7.encode("ascii")
    except UnicodeEncodeError as exc:
        raise ValueError("VIN must contain ASCII characters only") from exc
    if len(vin) != VIN_LEN:
        raise ValueError("VIN must be exactly seven characters")
    if not vin7.isalnum():
